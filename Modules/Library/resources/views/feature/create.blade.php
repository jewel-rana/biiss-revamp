        ...
        <div class="col-md-12">
            ...
            <div class="result-container">
                ...
                <div class="pull-left pl-3 m-t-3 m-b-20">
                    <form action="{{ url('dashboard/feature/create') }}" method="GET">
                        <input type="text" id="awesomplete" name="search" class="form-control input-white"
                               style="width:320px;" placeholder="Search by Title, Author, Type">
                ...
                <ul class="result-list" id="searchResult">
                    @if($items->count() > 0)
                        @foreach($items as $item)
                            <li>
                                    <?php
                                    $photo = ($item->cover_photo != null)
                                        ? asset($item->cover_photo)
                                        : asset('default/cover/' . strtolower($item->type) . '.jpg');
                                    ?>
                                <a href="{{ route('library.show', $item->id) }}" target="_blank" class="result-image"
                                ...
                                <div class="result-info">
                                    ...
                                    @if($item->type == 'journal')
                                        <p class="desc">
                                            <strong>VOL: </strong>{{ $item->volume_number ?? 'N/A' }}
                                            <strong>No: </strong>{{ $item->item_number }}
                                        </p>
                                        <p class="desc">
                                            <strong>Season: </strong>{{ $item->season ?? 'N/A' }}
                                            <strong>FREQ: </strong>{{ $item->friq }}
                                        </p>
                                    @endif
                                    <p class="desc">
                                        {{ $item->description }}
                                ...
                                <div class="result-price">
                                    @if(empty($item->featured))
                                        <a href="{{ url('dashboard/feature/add/' . $item->id . '/?type=' . $type) }}"
                                           class="btn btn-info btn-block">Add to list</a>
                                    @else
                                        <button type="button" class="btn btn-default btn-block">Added to top list
                                        </button>
                                    @endif
                                </div>
                            </li>
