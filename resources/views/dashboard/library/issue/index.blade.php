					...
					<div class="panel panel-inverse">
						...
						<div class="panel-heading">
							<div class="panel-heading-btn">
								<a href="{{ url('dashboard/issue/create') }}" class="btn btn-xs btn-circle btn-default"><i class="fa fa-plus"></i> Issue new Item</a>
							</div>
						...
						<div class="panel-body">
							...
							<table id="myDataTable" class="table table-striped table-bordered">
								...
								<tbody>
									...
									@php
										$coverPhoto = ( $issue->item['cover_photo'] != null ) ? asset( $issue->item['cover_photo'] ) : asset('default/cover/' . strtolower( $issue->item['type'] ) . '.jpg');
									@endphp
									<tr class="odd gradeX" id="parent">
										<td>
											<img src="{{ $coverPhoto }}" class="img-fluid" />
										</td>
										<td>
											<a href="{{ url('dashboard/library/item/' . $issue->item_id ) }}" target="_blank">
										...
										<td>
											<a href="{{ url('dashboard/issue/extend/' . $issue->id ) }}" class="btn btn-sm btn-block mb-2 btn-info" title="Re-Issue">
												<i class="fa fa-redo"></i> Re-issue
											</a>
											<a href="{{ url('dashboard/issue/return/' . $issue->id ) }}" class="btn btn-sm btn-block btn-success takeReturn" id="{{ $issue->stock['id'] }}" data-copy="{{ $issue->stock['copy_number'] }}" title="Take Return">
