        ...
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <div class="new_book">
                        <!-- Set up your HTML -->
                        <h2><span>BIISS {{ ucwords(str_replace('_', ' ', $type) ?? 'Book') }}</span></h2>
                        <div class="row pt-2">
                            ...
                            <table class="table table-striped table-bordered">
                                ...
                                @foreach( $featured as $feature )
                                    <tr>
                                        <td>
                                            @if( $feature->item->cover_photo != '' )
                                                <img src="{{ asset($feature->item->cover_photo) }}" class="img-fluid"
                                                     style="width:60px;" alt="">
                                            @else
                                                <img
                                                    src="{{ asset('default/cover/' . strtolower( $feature->item->type ) . '.jpg') }}"
                                                    class="img-fluid" style="width:60px;">
                                            @endif
                                        </td>
                                        <td>
                                            <a href="{{ url('/single/' . $feature->item->id ) }}">{{ $feature->item->title }}</a>
                                        </td>
                                        <td>
                                                <?php
                                                $articles = '';
                                                $subjects = '';
                                                if ($feature->item->authors) :
                                                    foreach ($feature->item->authors as $author) :
                                        ...
                                        <td>
                                                <?php
                                                if (empty($subjects)) {
                                                    if (!empty($feature->item->tags)) {
                                                        foreach ($feature->item->tags as $tag) {
                                                            if (!empty($subjects)) {
                                                                $subjects .= ', ';
                                                            }
                                        ...
                                        <td>{{ (int) $feature->item->publication_year }}</td>
                                        <td>
                                            @if($feature->item->hasEResource())
                                                <a href="{{ route('library.reader', [$feature->item->type, $feature->item->id]) }}"
                                                   target="_blank">
                                                    <img src="/frontend/images/pdf-svgrepo-com.svg" width="80"
                                                         ...
                                                         title="Read: {{ ucwords($feature->item->title) }}"
                                                         data-bs-toggle="tooltip" data-bs-placement="top"
                                                         alt="Read e-{{ ucfirst($feature->item->type) }}">
                                                </a>
                                            @endif
                                        </td>
                                    </tr>
                            ...
                            <nav>
                                {!! $featured->links('pagination::bootstrap-5') !!}
                            </nav>
                        </div>
